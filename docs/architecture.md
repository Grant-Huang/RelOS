...
- `ContextBlock`：结构化 Markdown 文本块
- `DecisionPackage`：复合场景结构化决策包
- `ActionBundle`：与 `decision_id` 绑定的 Shadow 动作包
...
- `AlarmEvent`：单告警事件 JSON
- `CompositeDisturbanceEvent`：复合扰动事件 JSON（订单/设备/物料/质量等多事件）
...
- 在 `ContextCompiler` 之上新增复合场景上下文构建能力
- 将多个子事件关联到统一时间窗与业务目标
- 输出面向上层 Agent 和前端演示的统一 `ContextBlock`

### 3.4 Decision Engine（决策引擎）

**职责**：基于图上下文，选择最优决策路径，输出根因推荐或结构化决策包。
...
生成 DecisionPackage
    ↓
requires_human_review ? 决策级 HITL : 直接进入 Shadow ActionBundle
```

`DecisionPackage` 关键字段：
...
- 单动作 `ActionRecord` 继续保留，用于原有告警分析路径
- 新增 `ActionBundle`，承载一个 `decision_id` 下的多动作 Shadow 计划
- 动作中仅保留 `target_system` 与 `payload_preview`，不直接执行外部系统写入
...
新增：
  ...
  ├── DecisionReviewRecord / ActionBundle
  ├── relos/decision/composite.py（复合场景上下文与决策包构建）
  ├── relos/decision/repository.py（决策包 / 动作包持久化）
